...
namespace Pixeltheory.Switchboard
{
   ...
   public abstract class PixelSocket<T> : PixelSocket where T : PixelSocket
   {
      ...
      #region Public
      public async Awaitable AddListener(PixelAction pixelAction, string listenerTypeName, int listenerID, CancellationToken ct)
      {
         ...
         if (!this.multicastListeners.ContainsKey(listenerTypeName))
         {
            this.multicastListeners.Add(listenerTypeName, ListPool<PixelAction>.Get());
         }
         ct.ThrowIfCancellationRequested();
         if (!this.multicastListeners[listenerTypeName].Contains(pixelAction))
         {
            this.multicastListeners[listenerTypeName].Add(pixelAction);  
         }
         ct.ThrowIfCancellationRequested();
      ...
      public async Awaitable RemoveListener(PixelAction pixelAction, string listenerTypeName, int listenerID, CancellationToken ct)
      {
         await PixelSocket<T>.asyncSemaphoreLock.WaitAsync(ct);
         ct.Register(() => { PixelSocket<T>.asyncSemaphoreLock.Release(); });
         if (this.broadcastListeners.Contains(pixelAction))
         ...
         if (this.multicastListeners.ContainsKey(listenerTypeName))
         {
            List<PixelAction> typeListeners = this.multicastListeners[listenerTypeName];
            ...
            {
               this.multicastListeners.Remove(listenerTypeName);
               ListPool<PixelAction>.Release(typeListeners);
            }
         }
         ct.ThrowIfCancellationRequested();
